const { ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, EmbedBuilder, ChannelType } = require('discord.js');
const fs = require('fs');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'modal_categoria') {
      const id = interaction.fields.getTextInputValue('categoria_id');
      fs.writeFileSync(path.join(__dirname, '../DataBaseJson/categoria.json'), JSON.stringify([id], null, 2));
      ...
      await interaction.reply({ content: `${emojis.confirmed_emoji} Categoria das filas atualizada!`, embeds: [embed], components: [], ephemeral: true });
    }
  }
}; 